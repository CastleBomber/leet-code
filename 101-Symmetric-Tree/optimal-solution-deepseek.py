...
********************************************************
    ...
    Notes:
        ...
        the iterative solution uses a queue.

    TIME AND SPACE COMPLEXITY: Symmetric Tree
    ----------------------------------------------------
    Metric           | Complexity | Reason
    ----------------------------------------------------
    Time Complexity  | O(n)       | Every node is visited exactly once (in the
                                    recursive or iterative process). n = total nodes.

    Space Complexity | O(n)       | Recursive: worst-case O(n) for skewed tree (call stack).
                                    Iterative: queue holds up to O(n) nodes in worst case
                                    (e.g., a complete tree at the last level).
    ----------------------------------------------------
...
class Solution:
    ...
    def _isMirror(self, left: Optional[TreeNode], right: Optional[TreeNode]) -> bool:
        # Base case: both nodes are None -> symmetric at this level
        if not left and not right:
            return True
        
        # If one of them is None and the other isn't: not symmetric
...
if __name__ == "__main__":
    # Build symmetric tree: [9,2,2,3,4,4,3]
    root = TreeNode(9,
                    TreeNode(2, TreeNode(3), TreeNode(4)), 
                    TreeNode(2, TreeNode(4), TreeNode(3)))

    sol = Solution()
    result = sol.isSymmetric(root)               # recursive approach
    result_iter = sol.isSymmetricIterative(root) # iterative approach
